...
B<mbus-serial-set-address> [-d] [-b BAUDRATE] device mbus-serial-number new-id
...
B<mbus-serial-set-address> - set primary addres for a mbus device.
...
=item B<mbus-serial-number>

Serial number of the mbus device

=item B<new-id>

New serial number of the mbus device
